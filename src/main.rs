...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(unicode_data) = user_selection {
        ...
        if let Some(output_type) = pargs
            .opt_value_from_str::<&str, String>("--output")
            .unwrap()
        {
            match output_type.as_str() {
                ...
                _ => {
                    println!(
                        "invalid display type \"{}\". Choose one of: symbol, codepoint, name",
                        output_type
                    );
                }
            }
            ...
            println!(
                "{} ({:#x}) - {}",
                &symbol, unicode_data.codepoint, unicode_data.name
            );
        }

        if pargs.contains("--copy") {
            let ctx_result: Result<ClipboardContext, Box<dyn std::error::Error>> =
